...
***MultipartRequest 객체의 생성자**************************************************
      MultipartRequest(
          javax.servlet.http.HttpServletRequest request, //서버에서 생성된 리퀘스트 (클라이언트 정보) 
          java.lang.String saveDirectory, //저장할 폴더 (실질적 저장 경로, 서버 경로) 
          int maxPostSize,
          java.lang.String encoding,
          FileRenamePolicy policy) //똑같은 파일 있을 때 어떻게 할까
     ...
     ***MultipartRequest 객체의 메서드**************************************************
     ...
     getFile() :  서버에 업로드 된 파일 객체 자체를 반환하는 메소드이다.
      ...
      물리적 경로(웹 서버 특정경로) :
  1. C:\SmartWeb\Web\WebJSP\JSPLab\WebServlet_93_FileUpload\WebContent\upload
  2. 이클립스 톰켓 내장 : 실 배포 경로
     실제 파일 저장 경로(아래 있는 경로)
    C:\SmartWeb\Web\WebJSP\JSPLab\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\WebServlet_93_FileUpload\upload
-->     
<%
	...
	//}	
	String file2 = (String)filenames.nextElement(); //name 이름 

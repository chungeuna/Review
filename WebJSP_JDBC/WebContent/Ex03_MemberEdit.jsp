...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<% 
	/* 회원정보 수정하기 
	1. DB 쿼리 : 2개 (수정 정보: select, 수정 정보 반영: update )
		1.1.  select * from koreamember where id = ? 
		1.2.  update koreamember set ename=?, pwd = ? ... where id = ?
		
	2. 화면 1개 (기존의 입력 내용을 보여준다)-> 처리 1개  (수정 처리 )
		2.1 DB select한 결과를 화면에 출력 
			<input type="text" value="rs.getString('id')">
		수정안하고 화면 : <td>rs.getString("id")</td>  >> 글자만 보여준다 >> 서버로 데이터 전송도 안함 
		수정 안하고 .. 화면 .. 전송 <input type="text" value="rs.getString("id")" readonly>

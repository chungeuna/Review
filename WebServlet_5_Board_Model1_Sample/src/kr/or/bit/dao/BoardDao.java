...
public class BoardDao {
	...
	public int deleteOK(String idx,String pwd) {
		...
		//계층형 게시판: 답글 
		/* 1. 원본글인데 답글이 있는 경우
		 * 2. 원본글인데 답글이 없는 경우 : 그냥 삭제 
		 * 3. 
		 * 
		 * 원본글(답글이 있는 경우) 
		 * case1: 원본글이 삭제되면 답변글도 있으면 다 삭제  (같은 ref  delete) 
		 * case2: (네이버)원본글만 삭제 -> 나머지 처리(텍스트 형태 (원본글 삭제표시) step, depth
		 * case3: 삭제시 -> 삭제되었음을 표시 (상태 컬럼: 삭제여부 컬럼 delok) 기본값 1 , 삭제하면 0  (설계를 다시해야..) > update 
		 * case4: 삭제 못하게 한다 > ref count > 1
		 */
		//case1 >> 원본글 삭제하면 답변글도 다 삭제 
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int row = 0;
		try {
			...
			//댓글부터 지운다 
			//두개의 테이블에서 fk가 있으면 자식부터 삭제하고 부모를 삭제해야한다 
			...
			if(rs.next()) { //삭제글은 존재하는 경우
				 // 사용자가 입력한 비번과 db 비번이 일치하는지를 본다 
				if(pwd.equals(rs.getString("pwd"))){
					//실 삭제 처리 
					//트랜잭션 
					//둘다 처리되거나 둘다 실패하거나 이 두개를 하나의 논리적 단위로 묶겠다 
					//JDBC :오토커밋 
					conn.setAutoCommit(false); //수동 커밋으로 바꾼다  개발자가 인위적으로 롤백과 커밋을 강제할 수 있다 
					//댓글 삭제 
					...
					if(row > 0) {
						conn.commit(); //두개의 delete 구문이 실반영 
					}  //else >> rollback은 의미가 없다 
				}else {
					row = -1; //비밀번호가 일치하지 않는 경우 
					
				}
			 }else { //삭제글이 존재하지 않는 경우
	...
	public int reWriteOk(Board boardata) {
		...
		try {
		    ...
		    //   원본글 답글     refer=1 , step=1+1 > 2 >3 , depth=1
			//	  원본글 답글  refer=1, step  = 1>2,    depth =1 
			//   원본들답글  refer=1 , step=1>1 , depth=1
